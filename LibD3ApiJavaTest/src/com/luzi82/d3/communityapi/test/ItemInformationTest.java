...
public class ItemInformationTest {
	...
	public void test_000() throws JsonParseException, JsonMappingException,
			IOException {
		ItemInformation ii = IO
				.readItemInformation("sample/iteminformation_000.json");
		...
		Assert.assertEquals(
				"item/ClsI7LypiQISBwgEFdMehaUdrNOZOR3zGKDgHZsGAMsdU6uOLB3zcAetHX5Wsy4wiwI41gFAAFASWARgmAOAAUalAVOrjiytAbi8IHq1AdrDjnG4AYX7w78BwAEFGNmE16cLUARYAqAB2YTXpwugAajO478CoAH4qs-wCw",
				ii.tooltipParams);
		...
		Assert.assertEquals(Const.AFFIXTYPE_DEFAULT,
				ii.attributes.primary[0].affixType);
		Assert.assertEquals("blue", ii.attributes.primary[0].color);
		Assert.assertEquals(Const.AFFIXTYPE_ENCHANT,
				ii.attributes.primary[4].affixType);
		Assert.assertEquals(2, ii.attributes.secondary.length);
		Assert.assertEquals(Const.AFFIXTYPE_UTILITY,
				ii.attributes.secondary[0].affixType);
		Assert.assertEquals(0, ii.attributes.passive.length);
		Assert.assertEquals(70,
				(int) ii.attributesRaw.Item_LegendaryItem_Level_Override.min);
		Assert.assertEquals(70,
				(int) ii.attributesRaw.Item_LegendaryItem_Level_Override.max);
		...
		Assert.assertEquals(23,
				(int) ii.attributesRaw.Damage_Delta_Physical.min);
		Assert.assertEquals(23,
				(int) ii.attributesRaw.Damage_Delta_Physical.max);
		Assert.assertEquals(0.07999999999999996,
				ii.attributesRaw.Power_Cooldown_Reduction_Percent_All.min,
				0.0001);
		Assert.assertEquals(0.07999999999999996,
				ii.attributesRaw.Power_Cooldown_Reduction_Percent_All.max,
				0.0001);
		Assert.assertEquals(1905181658,
				(int) ii.attributesRaw.Item_Legendary_Item_Base_Item.min);
		Assert.assertEquals(1905181658,
				(int) ii.attributesRaw.Item_Legendary_Item_Base_Item.max);
		...
		Assert.assertEquals(20181,
				(int) ii.attributesRaw.Health_Globe_Bonus_Health.min);
		Assert.assertEquals(20181,
				(int) ii.attributesRaw.Health_Globe_Bonus_Health.max);
		Assert.assertEquals(408,
				(int) ii.attributesRaw.Durability_Max_Before_Reforge.min);
		Assert.assertEquals(408,
				(int) ii.attributesRaw.Durability_Max_Before_Reforge.max);
		...
		Assert.assertEquals(14,
				(int) ii.attributesRaw.Resource_Max_Bonus_Arcanum.min);
		Assert.assertEquals(14,
				(int) ii.attributesRaw.Resource_Max_Bonus_Arcanum.max);
		Assert.assertEquals(0.08,
				ii.attributesRaw.Crit_Percent_Bonus_Capped.min, 0.0001);
		Assert.assertEquals(0.08,
				ii.attributesRaw.Crit_Percent_Bonus_Capped.max, 0.0001);
		Assert.assertEquals(2,
				(int) ii.attributesRaw.Item_Binding_Level_Override.min);
		Assert.assertEquals(2,
				(int) ii.attributesRaw.Item_Binding_Level_Override.max);
		...
		Assert.assertEquals("unique_amulet_007_x1_demonhunter_male",
				ii.set.items[0].icon);
		Assert.assertEquals("green", ii.set.items[0].displayColor);
		Assert.assertEquals("item/tal-rashas-allegiance",
				ii.set.items[0].tooltipParams);
		...
		Assert.assertEquals("Cold skills deal 5% more damage.",
				ii.set.ranks[0].attributes.primary[0].text);
		Assert.assertEquals("default",
				ii.set.ranks[0].attributes.primary[0].affixType);
		...
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Cold.min,
				0.0001);
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Cold.min,
		...
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Fire.min,
				0.0001);
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Fire.min,
		...
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Arcane.min,
				0.0001);
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Arcane.min,
		...
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Lightning.min,
				0.0001);
		Assert.assertEquals(
				0.05,
				ii.set.ranks[0].attributesRaw.Damage_Dealt_Percent_Bonus_Lightning.min,
		...
		Assert.assertEquals(
				2.0,
				ii.set.ranks[1].attributesRaw.Resource_Regen_Per_Second_Arcanum.min,
				0.0001);
		Assert.assertEquals(
				2.0,
				ii.set.ranks[1].attributesRaw.Resource_Regen_Per_Second_Arcanum.max,
				0.0001);
		Assert.assertEquals(
				2.0,
				ii.set.ranks[2].attributesRaw.Item_Power_Passive_ItemPassive_Unique_Ring_710_x1.min,
				1.0);
		Assert.assertEquals(
				2.0,
				ii.set.ranks[2].attributesRaw.Item_Power_Passive_ItemPassive_Unique_Ring_710_x1.max,
				1.0);
	...
	public void test_001() throws JsonParseException, JsonMappingException,
			IOException {
		ItemInformation ii = IO
				.readItemInformation("sample/iteminformation_001.json");
